...
def scatter_plot(df, df_name):
    axs = scatter_matrix(df, alpha=0.2, figsize=[20, 20])
    n = len(df.columns)
    for x in range(n):
        for y in range(n):
    ...
    plt.savefig('{}/plots/{}_scatter_matrix.png'.format(OUTPUT_DIRECTORY, df_name),
                format='png', dpi=150)
    plt.clf()


def box_plot(df, df_name, target_col, compare_col):
    df.boxplot(column=[compare_col], by=target_col, return_type='axes')
    plt.savefig('{}/plots/{}_{}_vs_{}_box_plot.png'.format(OUTPUT_DIRECTORY, df_name, compare_col, target_col),
                format='png', dpi=150)
    plt.clf()


def bar_plot(df, df_name, target_col, compare_col):
    fig = pd.crosstab(df[compare_col], df[target_col])
    fig.plot(kind='bar', figsize=[10, 10])
    plt.savefig('{}/plots/{}_{}_vs_{}_bar_plot.png'.format(OUTPUT_DIRECTORY, df_name, compare_col, target_col),
                format='png', dpi=150)
    plt.clf()
